(function ($) {
  ...
  var handleFlipCard = function (event) {
    ...
    if (status.code == 3) {
      ...
      var message = getEndGameMessage();
      const cardsArray = document.querySelectorAll(".flip-container.clicked");
      endingAnimation(cardsArray);
  ...
  function testFunc(winStatus) {
    buildLayout($.cards, $.settings.rows, $.settings.columns);
  }
  // Update on resize
  window.addEventListener("resize", testFunc, true);
...
const endingAnimation = function (cards) {
  for (let i = cards.length - 1; i >= 0; i--) {
    setTimeout(() => {
      console.log(i);
      cards[i].classList.add("cards-drop");
    }, 100 * i);
    setTimeout(() => {
      document.querySelector("#memory--cards").style.display = "none";
    }, 1000);
  }
};
